               ...
               map_file(map_file), load_map(load_map)
{
    ...
    else if(mSensor==MONOTOF)
        cout << "Monocular with Time-of-Flight" << endl;
    //set resolution for the point cloud TODO: change to set it in the launch file
    ...
    mpLocalMapper = new LocalMapping(mpMap, (mSensor==MONOCULAR || mSensor==MONOTOF));
    mptLocalMapping = new thread(&ORB_SLAM2::LocalMapping::Run,mpLocalMapper);

    //Initialize the Loop Closing thread and launch
    mpLoopCloser = new LoopClosing(mpMap, mpKeyFrameDatabase, mpVocabulary, (mSensor!=MONOCULAR || mSensor!=MONOTOF));
...
void System::TrackTOF(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
{
    if(mSensor!=TOF)
    {
        cerr << "ERROR: you called TrackTOF but input sensor was not set to TOF." << endl;
        exit(-1);
    }
    ...
    cv::Mat Tcw = mpTracker->GrabImageTOF(im,depthmap,timestamp);
...
void System::TrackMonoToF(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
{
    if(mSensor!=MONOTOF)
    {
        cerr << "ERROR: you called TrackMonoToF but input sensor was not set to MonoTOF." << endl;
        exit(-1);
    }
    ...
    cv::Mat Tcw = mpTracker->GrabImageMonoToF(im, depthmap, timestamp);
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    if(mSensor==MONOCULAR || mSensor==MONOTOF)
    {
