...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer,
        Map *pMap, shared_ptr<PointCloudMapping> pPointCloud, KeyFrameDatabase* pKFDB, const int sensor, ORBParameters& parameters):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpPointCloudMapping(pPointCloud), mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys),
    mpFrameDrawer(pFrameDrawer), mpMap(pMap), mnLastRelocFrameId(0), mnMinimumKeyFrames(5)
{
    ...
    if(sensor==System::MONOCULAR || sensor==System::MONOTOF)
    ...
    if(sensor==System::STEREO || sensor==System::RGBD || mSensor==System::TOF) //TODO MONOTOF PARAMETER
    {
        mThDepth = mbf*(float)mThDepth/parameters.fx;
        cout << endl << "Depth Threshold (Close/Far Points): " << mThDepth << endl;
    }

    if(sensor==System::RGBD || mSensor==System::TOF)
    {
...
cv::Mat Tracking::GrabImageTOF(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    mImRGB = imRGB;
    mImGray = imRGB;
    mImDepth = imD;
...
cv::Mat Tracking::GrabImageMonoToF(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    mImRGB = imRGB;
    mImGray = imRGB;
    mImDepth = imD;
...
void Tracking::Track()
{
    ...
    {
        if(mSensor==System::STEREO || mSensor==System::RGBD || mSensor==System::TOF)
...
void Tracking::UpdateLastFrame()
{
    ...
    if(mnLastKeyFrameId==mLastFrame.mnId || mSensor==System::MONOCULAR || mSensor==System::MONOTOF || !mbOnlyTracking)
        return;
...
bool Tracking::TrackWithMotionModel()
{
    ...
    if(mSensor!=System::STEREO)
        ...
        th=7;
    int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,th,mSensor==System::MONOCULAR || mSensor==System::MONOTOF);

    // If few matches, uses a wider window search
    if(nmatches<20)
    {
        fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));
        nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,mSensor==System::MONOCULAR || mSensor==System::MONOTOF);
    }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    int nTrackedClose= 0;
    if(mSensor!=System::MONOCULAR || mSensor!=System::MONOTOF)
    {
        for(int i =0; i<mCurrentFrame.N; i++)
        {
    ...
    const bool c1c =  (mSensor!=System::MONOCULAR || mSensor!=System::MONOTOF) && (mnMatchesInliers<nRefMatches*0.25 || bNeedToInsertClose) ;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    //Pass RGB image, ToF image and Pose to the pointcloud
...
void Tracking::SearchLocalPoints()
{
    ...
    if(nToMatch>0)
    {
        ...
        int th = 1;
        if(mSensor==System::RGBD || mSensor==System::TOF)
            th=3;
